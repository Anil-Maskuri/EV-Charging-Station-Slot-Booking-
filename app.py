...
def user_list():
    ...
    if session['user_type'] == 'admin':
        db=Db()
        qry = db.select("SELECT login_id, email, username FROM login WHERE usertype='user'")
...
def view_booking():
    ...
    if session['user_type'] == 'admin':
        db=Db()
        bookings = db.select("select booking_id	, booking_date, time_from, time_to, city, station_name, available_ports, login_id  from bookings1  order by booking_date desc;")
...
def adm_delete_booking(booking_id):
    ...
    if session['user_type'] == 'admin':
        db = Db()
        qry = db.delete("delete from bookings1 where booking_id='"+booking_id+"'")
...
def user_dashboard():
    if 'user_type' in session and session['user_type'] == "user":
        username = session['username'] # get the username from the session
        db = Db()
        bookings = db.select("select booking_date, time_from, time_to, city, station_name, available_ports, login_id ,booking_id  from bookings1 where login_id = '%s' order by booking_date desc;", (session['uid'],))
...
def usr_delete_booking(booking_date):
    if 'user_type' in session and session['user_type'] == "user":
        ...
        qry = db.delete("delete from bookings1 WHERE booking_date = %s", (booking_date,))
         # Retrieve the updated bookings
        bookings = db.select("SELECT booking_id, booking_date, time_from, time_to, city, station_name, available_ports, login_id FROM bookings1 WHERE login_id = %s ORDER BY booking_date DESC", (session['uid'],))
...
def book():
    if 'user_type' in session and session['user_type'] == 'user':
        ...
        sql = "insert into bookings1(station_name, city, available_ports, booking_date, time_from, time_to, login_id) VALUES (%s, %s, %s, %s, %s, %s, %s)"
        try:
            booking_id = db.insert(sql, (station_name, city, available_ports, booking_date, time_from, time_to, login_id))
